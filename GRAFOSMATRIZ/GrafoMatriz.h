...
public:												//métodos públicos
	...
	void setArco(Tipo a, Tipo b, int v);			//actualiza valor de arco recibiendo nombres de vertices
	...
	Vertice getVertice(Tipo a);						//devuelve atributos de un vertice, conociendo su nombre
	int getArco(int va, int vb);					//devuelve el valor de un arco recibiendo numeros de vertices
	int getArco(Tipo a, Tipo b);					//devuelve el valor de un arco recibiendo nombres de vertices
	...
	bool adyacente(Tipo a, Tipo b);					//determina si dos vertices son adyacentes recibiendo sus datos
};
...
void GrafoMatriz::setVertice(Tipo a, Vertice v) {			//modifica los atributos de un vertice, conociendo su dato
...
void GrafoMatriz::setArco(Tipo a, Tipo b) {				//actualiza valor del arco almacenado en matriz de adyacencia, de dos vertices recibidos por su nombre
...
void GrafoMatriz::setArco(Tipo a, Tipo b, int v) {		//modifica el valor almacenado en matriz de adyacencia, de dos vertices recibidos por su nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice del segundo nombre recibido
	if ((va >= 0) && (vb >= 0)) {							//comprueba existencia de los dos vertices
...
Vertice GrafoMatriz::getVertice(Tipo a) {					//retorna todos los atributos del vértice si está almacenado en el grafo, conociendo su nombre
	int va;													//variable para contener numero de vertice
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
...
int GrafoMatriz::getArco(Tipo a, Tipo b) {				//retorna el valor del arco almacenado en la matriz de adyacencia para dos vértices que recibe como parámetro por nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
...
int GrafoMatriz::getNumVertice(Tipo v) {					//determina el numero de un vertice dentro del grafo, conociendo su valor
...
void GrafoMatriz::nuevoVertice(Tipo v) {					//crea un nuevo vertice recibiendo su dato
...
bool GrafoMatriz::adyacente(Tipo a, Tipo b) {			//determina si dos vertices forman un arco conociendo su nombre
	int va, vb;												//variables para contener numeros de los vertices recibidos por nombre
	va = getNumVertice(a);									//calcula numero de vertice del primer nombre recibido
	vb = getNumVertice(b);									//calcula numero de vertice del segundo nombre recibido
